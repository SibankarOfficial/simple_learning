...
function LessonReader({ lesson, catalog, sources, section }) {
  ...
  return <><header className="page-heading lesson-heading"><div className="eyebrow">TOPIC LESSON <Badge status={lesson.status} /></div><h1>{lesson.title}</h1><p>Understand the idea, try it yourself, and explain what happened.</p><p className="muted">Content reviewed {lesson.reviewedAt} · {lesson.estimatedMinutes.firstPass} min first pass · {lesson.estimatedMinutes.practice} min practice</p></header>
    ...
    {selected === 'learn' && <div className="lesson-body"><section><h2>Prerequisites</h2>{lesson.prerequisiteTopicIds.length > 0 ? <><p>Understand these topics first. Planned links show their place in the syllabus.</p><TopicLinks ids={lesson.prerequisiteTopicIds} catalog={catalog} /></> : <p>No earlier topic is required. Start with the explanation below.</p>}{/* The interactive prerequisite diagnostic is temporarily hidden. Keep its data in each lesson for a later guided-check flow. */}</section>
      <section><h2>What you will learn</h2><ul>{lesson.objectives.map(o => <li key={o.id}>{o.text}</li>)}</ul></section>
    ...
    {selected === 'examples' && <><p>Read the example, predict a change, and try it in the runtime described below. Complete React modules also include an isolated live preview.</p><p className="muted">{lesson.runtime}</p>{lesson.examples.map(example => <section className="example-section" key={example.id} id={`example-${example.id}`}><div className="section-heading"><h2>{example.title}</h2><span className="muted">{example.depth}</span></div><Code>{example.code}</Code>{example.kind === 'module' && <CodePlayground initialCode={example.code} title={example.title} />}<p>{example.explanation}</p><div className="hint"><strong>Try it yourself</strong><p>{example.tryThis}</p></div><Sources ids={example.sourceIds} sources={sources} /></section>)}</>}
    ...
    {lesson.miniAppIds.length > 0 && <section className="related"><h2>Put this topic to work</h2><p>Build a small app, then use the same concepts in an independent variation.</p>{lesson.miniAppIds.map(id => <a className="button" key={id} href={href(catalog.subject.id, 'mini-app', id)}>Open linked mini app</a>)}</section>}
  </>;
}
